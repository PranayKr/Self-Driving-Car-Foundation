	...
	float p_miss) 
{
	...
	for ( i = 0; i < beliefs.size(); i++)
	{
		for (j = 0; j < beliefs[i].size(); j++)
		{
			...
			{
				hit = true;
				newbeliefs.push_back(float(beliefs[i][j]) * (1.0*p_hit + (1 - 1.0)*p_miss));
			}
			else {
				hit = false;
				newbeliefs.push_back(float(beliefs[i][j])*(0.0*p_hit + (1 - 0.0)*p_miss));
			}
			
		}
	}

	int newgridrows = beliefs.size();

	int newgridcols = beliefs[0].size();

	int count = 0;

	vector<float> new_list;

	for (int k = 0; k < newgridrows; k++)
	{
		new_list.clear();

		for (int l = 0; l < newgridcols; l++)
		{
			new_list.push_back(newbeliefs[count]);
			count = count + 1;
		}

		newGrid.push_back(new_list);
	}
...
//	newGrid (height, vector<float>(width,0.0));
	...
	{
		for (size_t j = 0; j < beliefs[i].size(); j++)
		{
			float new_i = (i + dy) % width;

			float new_j = (j + dx) % height;

			newGrid[int(new_j)][int(new_i)] = beliefs[i][j];
		}
	}

...
def get_speeds(data_list):
    ...
    for records in data_list[1:]:
        
        time,disp,yaw_rate,accln = records
...
def get_headings(data_list):
    ...
    for recs in data_list[1:]:
        
        time,disp,yaw_rate,accln = recs
...
def get_x_y(data_list):
    ...
    theta_list =  get_headings(data_list)
    
    speed_list = get_speeds(data_list)
    ...
    for i in range(1,len(data_list)):
        ...
        delta_x = D *cos(theta_list[i])
        
        delta_y = D*sin(theta_list[i])
...
def show_x_y(data_list,increment=1):
    ...
    x_coord =[p[0] for p in xy]
    y_coord =[p[1] for p in xy]
    
    theta_list = get_headings(data_list)
    
    theta_x = np.cos(theta_list)
    
    theta_y = np.sin(theta_list)
    
    Q = plt.quiver(x_coord[::increment],
                   y_coord[::increment],
                   theta_x[::increment],
                   theta_y[::increment],
                   units='x',
